...
{
    // If we're in quotes, preserve all spaces
...
void handle_exit_status(t_shell *shell, char *result, t_expansion_state *state)
{
    char exit_status_str[16];
    
    ft_itoa_buf(shell->exit_status, exit_status_str);
    ft_strlcpy(result + state->j, exit_status_str, ft_strlen(exit_status_str) + 1);
