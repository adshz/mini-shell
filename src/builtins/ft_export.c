...
static char	*handle_quoted_value(char *value)
{
	...
	if (val_len >= 2 && value[0] == '"' && value[val_len - 1] == '"')
	{
		...
		ft_putstr_fd("DEBUG [handle_quoted_value]:Removed double quotes: [",
			STDERR_FILENO);
	...
	ft_putstr_fd("DEBUG [handle_quoted_value]: Preserving value as is: [",
		STDERR_FILENO);
	ft_putstr_fd(unquoted, STDERR_FILENO);
	ft_putstr_fd("]\n", STDERR_FILENO);
	return (unquoted);
...
static void	export_with_value(t_shell *shell, char *key, char *value)
{
	...
	ft_putstr_fd("\nDEBUG [export_with_value]: Processing key: [",
		STDERR_FILENO);
	...
	else
	{
		...
		ft_putstr_fd("DEBUG [export_with_value]: After quote handling: [",
			STDERR_FILENO);
