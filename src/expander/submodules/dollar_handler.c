char	*dollar_handler(t_shell *shell, char *str, size_t *i)
{
	...
	ft_putstr_fd("DEBUG [dollar]: Starting dollar handler\n", 2);
	(*i)++;
	if (!str[*i] || str[*i] == ' ' || str[*i] == '"' || str[*i] == '\'')
	{
		ft_putstr_fd("DEBUG [dollar]: Single $ found, returning as is\n", 2);
		return (ft_strdup("$"));
	}
	if (str[*i] == '?')
	{
		...
		ft_putstr_fd("DEBUG [dollar]: $? expansion: '", 2);
		ft_putstr_fd(ret ? ret : "(null)", 2);
		ft_putstr_fd("'\n", 2);
		return (ret);
	}
	ret = get_env_var(shell, str, i);
	ft_putstr_fd("DEBUG [dollar]: Variable expansion result: '", 2);
	ft_putstr_fd(ret ? ret : "(null)", 2);
	ft_putstr_fd("'\n", 2);
	return (ret);
}
