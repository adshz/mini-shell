...
char *get_suffix(char *dollar_pos)
{
    ...
    // Special case for $? - there is no suffix
    if (*(dollar_pos + 1) == '?')
        return (ft_strdup(""));

    var_end = dollar_pos + 1;
    ft_putstr_fd("DEBUG get_suffix - variable name starts with: [", STDERR_FILENO);
    ...
    // If we stopped at a path separator or any other character, include it in suffix
    if (*var_end)
    {
