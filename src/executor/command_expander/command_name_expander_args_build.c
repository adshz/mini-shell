...
*/
static int complete_args_update(t_ast_node *node, char **expanded_args, char **split_parts)
{
	// Free old arguments and update with new ones
	ft_free_array(node->args);
	node->args = expanded_args;
	free(split_parts);

	// Update node value to match the command name
	free(node->value);
...
int build_new_args_array(t_ast_node *node, char **split_parts)
{
    ...
    if (cmd_copy_remaining_args(expanded_args, node->args, split_count, orig_count))
    {
