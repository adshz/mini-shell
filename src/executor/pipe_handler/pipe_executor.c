...
/*   Updated: 2025/01/29 15:20:19 by szhong           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
#include "../executor.h"
...
int	execute_pipe(t_shell *shell, t_ast_node *node)
{
	...
	right_status = 0;
	shell->in_pipe = true;  // Set pipe mode flag
	if (setup_pipe(pipe_fd) != 0)
		return (1);

	// Fork both processes first
	left_pid = fork();
	if (left_pid == -1)
	{
		shell->in_pipe = false;  // Reset pipe mode flag on error
		return (handle_fork_error(pipe_fd));
	}
	
	if (left_pid == 0)
	{
		// Child process should have its own signal handling
	...
	// Parent should ignore signals while waiting
	signal(SIGINT, SIG_IGN);
	signal(SIGQUIT, SIG_IGN);

	// Wait for both processes and store their status
	waitpid(left_pid, &left_status, 0);
	waitpid(right_pid, &right_status, 0);

	// Restore signal handling
	init_signals();
	shell->in_pipe = false;  // Reset pipe mode flag

	// Handle signal termination for both processes
	if ((left_status & 0x7f) || (right_status & 0x7f))
	{
		if ((left_status & 0x7f) == SIGQUIT || (right_status & 0x7f) == SIGQUIT)
		{
			// Only print the message if this is the rightmost process that got SIGQUIT
	...
	// Return the exit status of the rightmost command
...
void	execute_pipeline(t_shell *shell, t_ast_node *node)
{
	...
	if (pid2 == -1)
	{
		close(pipe_fd[0]);
		close(pipe_fd[1]);
		kill(pid1, SIGTERM);  // Terminate first child if second fork fails
		waitpid(pid1, NULL, 0);
		return;
	}
	...
	// Wait for both children and handle their exit status
	waitpid(pid1, &saved_status, 0);
	waitpid(pid2, &status, 0);

	if (status & 0x7f)  // Check if process was terminated by a signal
		shell->exit_status = 128 + (status & 0x7f);
	else
		shell->exit_status = (status >> 8) & 0xff;  // Extract exit status
