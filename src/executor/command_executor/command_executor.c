...
/*   Updated: 2025/01/29 15:04:23 by szhong           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
#include "../executor.h"
#include "types.h"
...
int	execute_command(t_shell *shell, t_ast_node *node)
{
	...
	ft_putstr_fd("\nDEBUG: execute_command called with command: [", STDERR_FILENO);
...
static int handle_external_parent_process(t_shell *shell, pid_t pid, char *cmd_path)
{
	int status;
