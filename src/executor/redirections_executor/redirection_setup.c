...
void	setup_redirections(t_shell *shell, t_ast_node *node)
{
	...
	// First pass: Collect all heredoc content
	i = 0;
	while (i < redir_count)
	{
	...
	// Second pass: Set up all heredoc pipes
	i = 0;
	while (i < redir_count)
	{
		if (redir_nodes[i]->type == AST_HEREDOC)
	...
	// Third pass: Handle other redirections
	i = redir_count - 1;
	while (i >= 0)
	{
		if (redir_nodes[i]->type != AST_HEREDOC)
		{
			if (handle_redirection_type(redir_nodes[i], shell, saved_stdin, saved_stdout) != 0)
			{
				cleanup_and_exit(saved_stdin, saved_stdout, 1);
				return ;
			}
