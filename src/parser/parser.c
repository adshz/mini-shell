...
/*   Updated: 2025/02/17 20:47:05 by evmouka          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
 ...
 */
static bool	check_initial_pipe(t_token *tokens, t_shell *shell)
{
...
static bool	handle_parse_error(t_token *current, t_shell *shell)
{
...
t_ast_node	*parse(t_token *tokens, t_shell *shell)
{
	...
	if (!tokens)
		return (NULL);
	if (!check_initial_pipe(tokens, shell))
		return (NULL);
	current = tokens;
	ast = parse_expression(&current, shell);
	if (!ast && handle_parse_error(current, shell))
		return (NULL);
	if (current != NULL)
	{
		free_ast(ast);
